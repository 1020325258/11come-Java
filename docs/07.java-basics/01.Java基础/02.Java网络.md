...
当我们想实现一个生产者-消费者模型时，为了保证并发场景下的数据安全，在生产者和消费者之间需要一个共享缓冲区，通过控制共享缓冲区的访问来保证并发安全，常见的比如说使用 BlockingQueue，内部通过 ReentrantLock 和 Condition 来保证并发安全，但是他存在一个问题就是 **性能比较差！**

针对性能差可以采用无锁方案 CAS 来保证并发安全，比如 **ConcurrentLinkedQueue** 就是通过 CAS 来保证了线程安全，但是它内部的实现 **比较复杂** ，如果我们使用 CAS 实现一个生产者-消费者模型的话，那实现成本相当高
...
Disruptor 中还针对`伪共享问题` 给出了解决方案，接下来看一下他是如何解决的
...
这 3 个变量如果在同一个 cache line 中的话，假如此时有两个线程对这 3 个变量进行操作，比如线程 A 修改了 takeIndex 变量，那么就会导致这一个缓存行（cache line）失效，此时线程 B 去访问该缓存行的其他变量如 putIndex 时会无法命中缓存，需要从内存重新读取
...
public class PaddedFalseSharingTest {
    ...
    private static class PaddedCounter {
        ...
        // 填充 56 字节（7 个 long，每个 8 字节），避免不同 Counter 实例共享缓存行
