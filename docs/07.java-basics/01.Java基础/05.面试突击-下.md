...
- [网络通信面试实战](#网络通信面试实战)
- [硬件级别可见性问题面试实战](#硬件级别可见性问题面试实战)
- [Java 系统架构安全面试实战](#java-系统架构安全面试实战)
- [深挖网络 IO 面试实战](#深挖网络-io-面试实战)
- [分布式架构面试连环炮](#分布式架构面试连环炮)
- [性能优化面试实战](#性能优化面试实战)
- [最大工作线程数，默认200, 4核8g内存，线程数经验值800](#最大工作线程数，默认200-4核8g内存，线程数经验值800)
- [微信亿级朋友圈的社交系统设计](#微信亿级朋友圈的社交系统设计)
- [生产环境面试实战](#生产环境面试实战)
...
因为 JIT（Just-In-Time，即时编译器）在动态编译时，可能会发生指令重排导致错误，上述代码对应的字节码如下：

```BASH
 0 getstatic #2 <com/example/springbootpro/Singleton/Singleton.instance : Lcom/example/springbootpro/Singleton/Singleton;>
 3 ifnonnull 37 (+34)
 6 ldc #3 <com/example/springbootpro/Singleton/Singleton>
 8 dup
 9 astore_0
10 monitorenter
11 getstatic #2 <com/example/springbootpro/Singleton/Singleton.instance : Lcom/example/springbootpro/Singleton/Singleton;>
14 ifnonnull 27 (+13)
17 new #3 <com/example/springbootpro/Singleton/Singleton>
20 dup
21 invokespecial #4 <com/example/springbootpro/Singleton/Singleton.<init> : ()V>
24 putstatic #2 <com/example/springbootpro/Singleton/Singleton.instance : Lcom/example/springbootpro/Singleton/Singleton;>
27 aload_0
28 monitorexit
29 goto 37 (+8)
32 astore_1
...
35 aload_1
36 athrow
37 getstatic #2 <com/example/springbootpro/Singleton/Singleton.instance : Lcom/example/springbootpro/Singleton/Singleton;>
40 areturn
```
...
- new 创建一个对象，并将其引用压入栈顶：创建一个 Singleton 的实例对象，此时还没有执行对应的构造方法进行初始化
- dup 复制栈顶数值并将值压入栈顶：在调用 Singleton 初始化方法之前保留一份对象引用，在初始化过程中需要这个引用进行后续的静态变量赋值操作
- invokespecial 调用 Singleton 的初始化方法：调用构造方法进行初始化
- putstatic 将初始化后的 Singleton 赋值给静态变量 instance

在单线程下 `putstatic` 和 `invokespecial` 进行指令重排，可以提高效率；在 **多线程** 下，指令重排可能会出现意想不到的结果
...
- 多线程情况下，如果发生上述指令重排，此时第二个线程执行`getInstance`会执行到`if(instance==null)`，**此时会拿到一个尚未初始化完成的对象，之后使用未初始化完成的对象时可能会发生错误**


示例图如下：

![image-20241019172627984](https://11laile-note-img.oss-cn-beijing.aliyuncs.com/image-20241019172627984.png)
