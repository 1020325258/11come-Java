...
- **不支持事务** ，容易出现数据丢失
- **表级锁定** ，高并发场景下表现不好
- 数据、索引 **分离存储** ，检索数据更快，适合查询操作



MyISAM 不支持事务，因此更新操作可能会导致数据丢失，并且是表级锁定，锁的粒度太大了，因此 MyISAM 比较适合 **查询较多的场景** ，如下：
...
如果一张表适合完全放在内存中，则哈希索引可以将索引值转换为指针来直接查找到元素，以此提升查询速度，InnoDB 有一个监视索引搜索的机制，如果 InnoDB 注意到通过哈希索引查询可以变得更快，它就会自动创建
...
explain 执行计划可以让研发人员快速判断 SQL 对应的性能，以及具体的执行情况，因此掌握 explain 的使用是比较重要的！
接下来会通过几种示例来展示如何通过 explain 执行计划来判断 SQL 是否走到了索引，通过对应案例可以更快理解 explain 执行计划的使用
...
## 有排查过慢 SQL 吗？一般怎么进行排查？

这里分享一下慢 SQL 的高频面试问题：**是否排查过慢 SQL ，怎么排查慢 SQL？**

（一般来说，公司都会提供慢 SQL 告警，所以这个问题的重点是，在收到慢 SQL 告警之后，怎么针对具体的 SQL 去进行问题排查）



排查慢 SQL，一般使用 explain 去分析 SQL 语句，看几个关键关键的列，即 type、possible key、key、key_len、extra 这几个列，根据执行计划的结果，去分析 SQL 是否命中索引

如果没有命中索引，就会进行全表扫描，导致速度过慢，因此需要分析为什么没有走到索引，在（采用左模糊匹配）、（没有遵循最左前缀原则）、（索引列使用函数）、（联合索引第一个字段采用范围查询）等情况下都会导致索引失效

如果命中索引的话，就需要从其他方面分析问题：

- 技术层面

大量回表查询：在命中索引情况下，如果同时满足 `没有设计覆盖索引 && 二级索引的选择性不强` ，就会出现大量的回表查询，从而导致 SQL 查询过慢

比如通过 **二级索引** 筛选数据，但是筛选出来的数据很多，并且没有走到覆盖索引，因此大量的数据需要回表查询，拿到完整的数据之后，如果进行 order by 还需要对大量数据进行磁盘排序，时间开销太大，这类 SQL 优化可以将 SELECT * 改为 SELECT 字段1，字段 2 ，设计好覆盖索引，在二级索引就可以找到对应的数据，不需要回表查询以及磁盘排序

- 设计层面

技术上的优化是有限的，因此也要从设计上来看是否合理，比如单表数据量是否太大、数据库节点的 CPU、网络 IO、磁盘 IO 负载过高



**总结：** 慢 SQL 的排查，首先看是否走索引、索引设计是否合理，其次看是否查询了大量无效数据，并进行很多不必要的计算

这个问题考察的比较全面，还涉及到了其他很多知识点：什么情况下索引会失效？索引的设计原则？什么情况下回表查询？分库分表设计？



美团 SQL 慢查询优化：https://tech.meituan.com/2014/06/30/mysql-index.html
...
如果一行数据没有索引，InnoDB 会自动创建一个隐藏列 ROWID 的聚簇索引，会在这个聚簇索引上加锁
...
- 为什么对于 `count(id)`，mysql最终选择辅助索引而不是主键聚集索引？因为二级索引相对主键索引存储数据更少，检索性能应该更高，mysql内部做了点优化（在5.7版本才优化）。
...
MVCC（Multi-Version Concurrency Control） 出现的核心目的就是解决读写操作阻塞的问题，提高事务执行的并发能力

### 工作原理

MVCC 是基于 **undolog**、**版本链**、**ReadView** 实现的，通过记录每一行数据的快照版本，实现对同一行记录的不同版本数据的读取

在事务开启时，会生成一个事务 ID，通过比较当前事务的事务 ID 和数据行不同版本的事务 ID，来判断到底读取数据行哪一个快照版本的数据



InnoDB 在每一行数据中都会添加三个隐藏字段：

- trx_id（事务 ID，最后一次更新该行的事务 ID）
- row_id（如果表中没有主见或唯一非空索引，则会创建 6 字节的 row_id）
- roll_pointer（回滚指针，指向该行数据更新前的 undo log 地址）



如下图，在每次更新或者删除数据时，都会将更新前的数据记录在 `undo 日志`中，每条 `undo 日志` 通过 `roll_pointer` 进行关联，构成了数据的`版本链`

<img src="https://11laile-note-img.oss-cn-beijing.aliyuncs.com/image-20241004151639325.png" alt="image-20241004151639325" style="zoom:80%;" />
...
ReadView 记录了事务开启时，当前所有事务的集合，包含了最大的事务 ID 和最小的事务 ID

对于事务中执行的查询操作，通过当前的事务 ID 和当前行数据不同版本的事务 ID 比较，来选择当前事务可见的数据
...
- 执行 start transaction 之后，并不会立即生成事务 id，而是在该事务中，第一次修改 InnoDB 数据时才会为该事务生成事务 id
...
**Buffer Pool** 是 InnoDB 存储引擎中的一个很重要的内存结构，MySQL 的增删改查都是直接操作 BufferPool 的，一般设置 BufferPool 的大小为机器内存的 60% ，Buffer Pool 的大小在 `/etc/my.cnf` 中进行配置：
...
1、在 Server 层先进行 SQL 的解析、优化

2、接下来到了存储引擎层，如果需要更新的数据不在缓冲池中，就先去磁盘中加载数据页，把需要修改数据所在的数据页，缓存到 BufferPool，`BufferPool 中缓存的其实就是一个个的数据页`

3、修改记录前，写 undo 日志，记录更改前数据

4、更新 Buffer Pool 中的数据，并且将 redo log 写到 redo log buffer 中

5、提交事务时，会做两件事情：将 redo buffer 中的 redo log 刷到磁盘中去，保存操作记录；将 bin log 日志写入到磁盘，保存操作记录

- 先将 redo log 的数据刷到 redo log buffer 中，再写 bin log，之后去将 bin log 同步到磁盘

- 当 bin log 成功写入到磁盘后，会将 bin log 的文件名以及 bin log 日志在文件里的位置都写入到 redo log 中去，并且在 redo log 里写入一个 commit 标记，代表完成了事务的提交

6、数据持久化，IO 线程不定期把 Buffer Pool 中的脏数据（未持久化数据）刷入到磁盘，完成持久化



**在 redo log 中写入 commit 标记的原因：**

在提交事务时，会先持久化 bin log，之后在 redo 日志中记录 commit 标记，这是为了 **保证事务提交之后 redo log 与 bin log 数据一致性**

因为 MySQL 是插件式的数据库，可以同时使用多个存储引擎，如果提交事务时，先在 redo log 中写入 commit 标记，再去持久化 bin log，假如 bin log 没来得及持久化 MySQL 就宕机了，那么 InnoDB 存储引擎可以根据 redo log 来恢复数据，但是其他的存储引擎没有 redo log，而且 bin log 没有成功写入，因此无法根据 bin log 恢复数据



### bin log、redo log、undo log 作用：

**redo 日志和 undo 日志是 InnoDB 引擎特有的，而 bin log 是属于 Server 层的，与存储引擎无关**

- bin log：bin log 叫做归档日志，主要记录了做了什么操作，比如更新一行数据，bin log 就记录了对哪一行数据做更改，更新前的值是什么，更新为了什么值。bin log 日志是用于 MySQL 的主从复制和数据恢复

- redo log：用于恢复 BufferPool 中的数据，也就是用来恢复不完整提交事务写入的数据。在提交事务之前，会先使用 redo log 持久化事务中修改的数据，当 redo log 落盘并且打上 commit 标记就代表事务已经持久化完成
- undo log：存放事务修改前的旧数据，用于进行数据的回滚
...
而 redo log 就是用于恢复 BufferPool 中的数据，在 BufferPool 中修改数据后，会将 redo log 写入到 redo log buffer 中，此时 redo log 还没有刷到磁盘中去，如果 MySQL 宕机的话，redo log 也会丢失，而当提交事务之后，会将 redo log 刷到磁盘中去，此时 MySQL 宕机的话，也可以根据磁盘中的 redo log 将这一次的更新操作给恢复到 BufferPool 中
...
- **磁盘写性能差：** 如果直接将修改的数据刷入磁盘的话，会触发磁盘的 **随机读写** ，这个操作是很慢的，如果是提交大事务，就会导致大量的随机读写，数据库就会有抖动



![1707462191681](https://11laile-note-img.oss-cn-beijing.aliyuncs.com/1707462191681.png)

**因此 redo log 提供了高性能的写操作，并且还可以支持 BufferPool 中的数据恢复**

接下来说一下 redo log 是如何实现这两个功能

- **高性能的写操作：** 当我们去 MySQL 中更新数据，不直接将数据刷到磁盘中去，而是先将更新数据的操作给记录到 redo log buffer 中去，再根据 redo log 的刷盘策略写到磁盘中去，通过磁盘顺序写来提升写性能
  - 日志格式大概为：对表空间 XX 中的数据页 XX 中的偏移量为 XX 的地方更新了数据 XX，**将 redo log 刷入磁盘使用的是磁盘的顺序写**，性能几乎和内存随机读写相当

- **BufferPool 数据恢复：** 如果 MySQL 宕机，但是 redo log 已经持久化到磁盘，那么可以通过 redo log 还可以重做恢复 Buffer Pool 中的数据
...
因为 redo log 记录的是为持久化的事务修改的数据，那么比较早的事务修改的数据肯定都已经持久化到磁盘了，留着他们的没有太大的意义，因此 MySQL 默认设置了两个 redo log 文件，交替去写，这样不会让太多无用的数据占用大量的磁盘空间

**redo log 持久化流程：** 默认情况下 redo log 会写入到一个日志目录中去，在该日志目录下会写 redo log，默认会有两个 redo log 的磁盘文件，每个文件大小为 48MB
...
[MySQL集群文章](https://juejin.cn/column/7106086228201570312)
