...
接下来说一说 MySQL 如何解决幻读问题

首先幻读是指在一个事务内两次查询结果不一致，在 MySQL 中有两种查询：

- 快照读：select ...
  - 通过 MVCC 解决幻读问题，这种方式在查询时会根据当前的事务 id，只获取在当前事务之前插入的数据，因此不会读取到其他事务提交的数据，从而造成幻读
- 当前读：select ... for update
  - 通过临键锁解决幻读问题，在数据行之间加锁，来避免其他事务向相关的数据行之间插入数据，从而造成幻读



但是 MySQL 通过上边两种方式只是 **在一定程度上解决了幻读** ，在极端场景下还是会出现幻读问题，如下两种情况：
...
这两种情况 **本质原因** 是更新数据和当前读时，都会去更新版本号，因此会读取到最新数据，造成幻读
...
<img src="https://11laile-note-img.oss-cn-beijing.aliyuncs.com/1705415508036.png" alt="1705415508036" style="zoom:80%;" /> 
...
- 行级锁：锁一行记录的索引
  - 记录锁：锁定索引的一条记录
  - 间隙锁：锁定一个索引区间
  - 临键锁：记录锁和间隙锁的结合，`临键锁 + MVCC 解决幻读问题`
